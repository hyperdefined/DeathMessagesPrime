...
class DeathListener implements Listener {
    ...
    public void onPlayerDamaged(final EntityDamageEvent e) {
        if (e.getEntity() instanceof Player) {
            ...
            p.setMetadata(
                    "dmp.lastCause",
                    new FixedMetadataValue(this.pl, e.getCause().toString()));
    ...
    public void onPlayerMove(final PlayerMoveEvent e) {
        ...
        if (mcVer(1_016)) {
            ...
            check = m == Material.LADDER
                    || m == Material.VINE
                    || m == Material.WATER
                    || materialSafeCheck(m, "STATIONARY_WATER");
        }
        if (check) {
    ...
    TextComponent format(
            ...
            final String s,
            final String killer_name,
            final String weapon_name,
            final ItemStack is) {
    ...
    TextComponent formatV(
            ...
            try {
                ...
                if (cc != null && cc.contains(mt)) {
                    ...
                    if (cc2 != null) {
                        ...
                        if (ls.size() < 1) messageToUse = "";
                        else messageToUse = ls.get(r.nextInt(ls.size()));
                    }
                }
            } catch (Exception ex) {
            }
        ...
        if (shouldFetch) {
            try {
                ...
                if (A != null) {
                    ConfigurationSection B = A.getConfigurationSection(
                            e.getEntity().getUniqueId().toString());
                    if (!B.contains(ar)) ar = "*";
                    if (B.contains(ar)) {
                        List<String> dmsgs = B.getStringList(ar);
                        if (dmsgs.size() < 1) messageToUse = "";
                        else messageToUse = dmsgs.get(r.nextInt(dmsgs.size()));
                    }
                }
            } catch (Exception ex) {
            }
                ...
                try {
                    ...
                    if (A != null) {
                        ConfigurationSection B =
                                A.getConfigurationSection(k.getUniqueId().toString());
                        if (!B.contains(ar)) ar = "*";
                        if (B.contains(ar)) {
                            List<String> dmsgs = B.getStringList(ar);
                            if (dmsgs.size() < 1) messageToUse = "";
            ...
            try {
                ...
                if (A != null) {
                    ConfigurationSection B =
                            A.getConfigurationSection(e.getEntity().getWorld().getName());
                    if (!B.contains(ar)) ar = "*";
                    if (B.contains(ar)) {
                        List<String> dmsgs = B.getStringList(ar);
                        if (dmsgs.size() < 1) messageToUse = "";
    ...
    String handleStandardPlaceholders(
            String text,
            Player p,
            String killer_name,
            String killer_name2,
    ...
    @SuppressWarnings("deprecation")
    TextComponent handleSpecialPlaceholders(
            String t,
            Player p,
            ItemStack is,
            String weapon_name,
            ...
            if (txt.contains("%")) {
                ...
                while (true) {
                    ...
                    if (tag.equals("weapon")) {
                        if (is == null) {
                            result = new TextComponent(TextComponent.fromLegacyText(
                                    (pl.config.getBoolean("death-message-show-rarity", true) ? getRarityColor(is) : "")
                                            + ((weapon_name != null && weapon_name.length() > 0)
                                                    ? weapon_name
                                                    : itemStackNameToString(is))));
                        } else {
                            result = new TextComponent(TextComponent.fromLegacyText(
                                    (pl.config.getBoolean("death-message-show-rarity", true) ? getRarityColor(is) : "")
                                            + ((weapon_name != null && weapon_name.length() > 0)
                                ...
                                if (hasPiglinBrute())
                                    result.setHoverEvent(new HoverEvent(
                                            ...
                                            new net.md_5.bungee.api.chat.hover.content.Item(
                                                    is.getType().getKey().toString(),
                                                    is.getAmount(),
                                                    ItemTag.ofNbt(getItemStackNBTTag(is)))));
                                else {
                        ...
                        try {
                            ...
                            result = new TextComponent(TextComponent.fromLegacyText(
                                    (killer_name2.length() < 1 ? killer_name : killer_name2)));
                            hvt = HoverEvent.Action.SHOW_ENTITY;
                            if (!hasPiglinBrute()) hover = prepareEntityJson(convertEntityToJson(damager, killer_name));
                            else
                                result.setHoverEvent(new HoverEvent(
                                        hvt,
                                        new net.md_5.bungee.api.chat.hover.content.Entity(
                                                damager.getType().getKey().toString(),
                                                damager.getUniqueId().toString(),
                                                new TextComponent(TextComponent.fromLegacyText(killer_name)))));
                        } catch (Throwable t_) {
                            if (pl.debug) t_.printStackTrace();
                        ...
                        try {
                            hvt = HoverEvent.Action.SHOW_ENTITY;
                            if (!hasPiglinBrute()) hover = prepareEntityJson(convertEntityToJson(p, p.getName()));
                            else
                                result.setHoverEvent(new HoverEvent(
                                        hvt,
                                        new net.md_5.bungee.api.chat.hover.content.Entity(
                                                p.getType().getKey().toString(),
                                                p.getUniqueId().toString(),
                                                new TextComponent(TextComponent.fromLegacyText(p.getName())))));
                        } catch (Throwable t_) {
                            if (pl.debug) t_.printStackTrace();
                        }
                            ...
                            if (!hasPiglinBrute())
                                ...
                                result.setHoverEvent(new HoverEvent(
                                        ...
                                        new net.md_5.bungee.api.chat.hover.content.Entity(
                                                p.getType().getKey().toString(),
                                                p.getUniqueId().toString(),
                                                new TextComponent(TextComponent.fromLegacyText(p.getDisplayName())))));
                        } catch (Throwable t_) {
                            if (pl.debug) t_.printStackTrace();
                        }
                    } else if (tag.equals("")) {
                        ...
                        if (p.hasMetadata("dmp.lastCause")
                                && p.getMetadata("dmp.lastCause").size() > 0) {
                            dam = EntityDamageEvent.DamageCause.valueOf(
                                    p.getMetadata("dmp.lastCause").get(0).asString());
                        }
                        ...
                        for (String k : DeathMessagesPrime.taglistenerprefixes.keySet()) {
                            if (tag.startsWith(k)) {
                                result = DeathMessagesPrime.taglistenerprefixes
                                        .get(k)
                                        .formatTag(tag, p, dam, damager);
                                break;
                            }
                        }
                    }
                    ...
                    if (hvt != null && hover != null) {
                        ...
                        else result.setHoverEvent(new HoverEvent(hvt, new BaseComponent[] {new TextComponent(hover)}));
                    }
        ...
        if (ComponentSerializer.toString(message).length() > 30000) { // too long still
            message.setExtra(Arrays.asList(
                    TextComponent.fromLegacyText(message.toLegacyText().substring(256))));
        }
        return message;
    }
    ...
    TextComponent format(
        ...
        String tp = handleStandardPlaceholders(
                pref, p, killer_name, killer_name2, weapon_name, is, killer_xp_ok, killer_xp, killer_level, rdist);
        String ts = handleStandardPlaceholders(
                suff, p, killer_name, killer_name2, weapon_name, is, killer_xp_ok, killer_xp, killer_level, rdist);
        String t = handleStandardPlaceholders(
                s, p, killer_name, killer_name2, weapon_name, is, killer_xp_ok, killer_xp, killer_level, rdist);
    ...
    private Object getBiome(Player p) {
        if (mcVer(1_015)) {
            ...
            return p.getWorld()
                    .getBiome(p.getLocation().getBlockX(), p.getLocation().getBlockZ());
        }
    }
    ...
    private int getRarity(ItemStack is) {
        ...
        if (!is.getEnchantments().isEmpty()) {
            if (materialSafeCheck(ism, "ENCHANTED_BOOK")) {
        ...
        } else if (materialSafeCheck(ism, "EXPERIENCE_BOTTLE")
                || materialSafeCheck(ism, "LEGACY_EXP_BOTTLE")
                || materialSafeCheck(ism, "DRAGON_BREATH")
                || materialSafeCheck(ism, "LEGACY_DRAGONS_BREATH")
                || materialSafeCheck(ism, "ELYTRA")
                || materialSafeCheck(ism, "LEGACY_ELYTRA")
                || materialSafeCheck(ism, "NETHER_STAR")
                || materialSafeCheck(ism, "LEGACY_NETHER_STAR")
                ...
                || materialSafeCheck(ism, "CREEPER_HEAD")
                || materialSafeCheck(ism, "SKELETON_SKULL")
                || materialSafeCheck(ism, "LEGACY_SKULL")
                || materialSafeCheck(ism, "WITHER_SKELETON_SKULL")
                || materialSafeCheck(ism, "HEART_OF_THE_SEA")) {
            return 1; // uncommon
        } else if (materialSafeCheck(ism, "GOLDEN_APPLE")
                || materialSafeCheck(ism, "LEGACY_GOLDEN_APPLE")
                || materialSafeCheck(ism, "BEACON")
                || materialSafeCheck(ism, "LEGACY_BEACON")
                || materialSafeCheck(ism, "END_CRYSTAL")
                || materialSafeCheck(ism, "LEGACY_END_CRYSTAL")
                || materialSafeCheck(ism, "CONDUIT")
                || ism.name().startsWith("MUSIC_DISC_")) {
            return 2; // rare
        } else if (materialSafeCheck(ism, "COMMAND_BLOCK")
                || materialSafeCheck(ism, "LEGACY_COMMAND")
                || materialSafeCheck(ism, "REPEATING_COMMAND_BLOCK")
                || materialSafeCheck(ism, "LEGACY_COMMAND_CHAIN")
                || materialSafeCheck(ism, "CHAIN_COMMAND_BLOCK")
                || materialSafeCheck(ism, "LEGACY_COMMAND_REPEATING")
                || materialSafeCheck(ism, "DRAGON_EGG")
                || materialSafeCheck(ism, "LEGACY_DRAGON_EGG")
                || materialSafeCheck(ism, "STRUCTURE_BLOCK")
                || materialSafeCheck(ism, "LEGACY_STRUCTURE_BLOCK")
    ...
    private void copyFormatting(BaseComponent t, BaseComponent s) {
        if (mcVer(1_008)) copyFormattingModern(t, s);
        else copyFormattingClassic(t, s);
    }
    ...
    private String getBiomeName(String b) {
        try {
            ...
            for (String k : c.getKeys(false)) {
                if (k.equals(b)) return c.getString(k);
            }
        } catch (Exception ex) {
        }
    ...
    private String getWorldName(String w) {
        try {
            ...
            for (String k : c.getKeys(false)) {
                if (k.equals(w)) return c.getString(k);
            }
        } catch (Exception ex) {
        }
        return w;
    ...
    private TextComponent handleCustomMessages(
            PlayerDeathEvent e,
            Player p,
            Entity damager,
            boolean isPvP,
            String section,
            String death_tag,
            boolean use_player_for_get_killer_name) {
        TextComponent deathmsg;
        String checkname = this.getCheckName(damager, p, false);
        if (isPvP) {
            deathmsg = handleCustomPlayerMessages(
                    e, p, damager, section, death_tag, checkname, use_player_for_get_killer_name);
        } else {
    ...
    private TextComponent handleCustomPlayerMessages(
            PlayerDeathEvent e,
            Player p,
            Entity source,
            String section,
        ...
        if (this.fc.isConfigurationSection("custom-user-death-messages")) {
            ConfigurationSection cc = this.fc
                    .getConfigurationSection("custom-user-death-messages")
            ...
            if (cc != null)
                for (String rx : cc.getKeys(false)) {
                    if (checkname.matches(rx.replace("_", "."))) {
                        ...
                        deathmsg = this.formatV(
                                e,
                                source,
                                death_tag,
                                ls.get(r.nextInt(ls.size())),
                                this.getKillerName(source, use_player_for_get_killer_name ? p : null),
                                this.getKillerName2(source, use_player_for_get_killer_name ? p : null),
                                "");
                        break;
                    }
                }
        ...
        if (deathmsg == null) {
            ...
            if (this.fc.isConfigurationSection("custom-player-death-messages")) {
                ConfigurationSection cc = this.fc
                        .getConfigurationSection("custom-player-death-messages")
                        .getConfigurationSection("Potion");
                if (cc != null)
                    for (String rx : cc.getKeys(false)) {
                        if (checkname.matches(rx.replace("_", "."))) {
    ...
    private TextComponent handleCustomMobMessages(
            PlayerDeathEvent e,
            Player p,
            Entity le,
            String section,
        ...
        if (this.fc.isConfigurationSection("custom-mob-death-messages")) {
            ...
            ConfigurationSection cc =
                    this.fc.getConfigurationSection("custom-mob-death-messages").getConfigurationSection(section);
            if (cc != null)
                for (String rx : cc.getKeys(false)) {
                    if (checkname.matches(rx.replace("_", "."))) {
                        List<String> ls = cc.getStringList(rx);
                        ...
                        deathmsg = this.formatV(
                                ...
                                death_tag,
                                ls.get(r.nextInt(ls.size())),
                                this.getKillerName(le, use_player_for_get_killer_name ? p : null),
                                this.getKillerName2(le, use_player_for_get_killer_name ? p : null),
                                "");
                        break;
                    }
                }
    ...
    synchronized void onPlayerDeath_i(final PlayerDeathEvent e) {
        ...
        } else if (p.hasMetadata("dmp.lastCause")
                && p.getMetadata("dmp.lastCause").size() > 0) {
            d = EntityDamageEvent.DamageCause.valueOf(
                    p.getMetadata("dmp.lastCause").get(0).asString());
        }
        ...
        try {
            vdeathmsg = "Death[Player=" + p.getName() + ",LastCause=" + d.toString() + ",Damager="
                    + (p.hasMetadata("dmp.lastDamage")
                            ? (damager + "(" + this.getKillerName(damager, p) + ")")
                            : (p.hasMetadata("dmp.lastDamageEx")
                                    ? (damager + "(" + this.getKillerName(damager, p) + ")EX")
                                    : "null"))
                    + ",Location=[" + l.getWorld().getName() + "](" + l.getBlockX() + "," + l.getBlockY() + ","
                    + l.getBlockZ() + ")";
        } catch (Exception ex) {
            try {
                vdeathmsg = "Death[Player=" + p.getName() + ",LastCause=" + d.toString() + ",Damager=???,Location=["
                        + l.getWorld().getName() + "](" + l.getBlockX() + "," + l.getBlockY() + "," + l.getBlockZ()
                        + ")";
            } catch (Exception ex2) {
                vdeathmsg = "Death[Player=" + p.getName() + ",LastCause=???,Damager=???,Location=["
                        + l.getWorld().getName() + "](" + l.getBlockX() + "," + l.getBlockY() + "," + l.getBlockZ()
                        + ")";
            }
        }
        ...
        if (d == EntityDamageEvent.DamageCause.STARVATION) {
            deathmsg = this.formatV(
                    e, death_tag = "natural.Starvation", this.getMessage("death-messages.natural.Starvation"), "");
        } else if (d == EntityDamageEvent.DamageCause.SUFFOCATION) {
            deathmsg = this.formatV(
                    e, death_tag = "natural.Suffocation", this.getMessage("death-messages.natural.Suffocation"), "");
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "ElytraKill", " natural.ElytraKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.ElytraKill",
                            this.getMessage("death-messages.natural.ElytraKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
            } else {
                deathmsg = this.formatV(
                        e, death_tag = "natural.Elytra", this.getMessage("death-messages.natural.Elytra"), "");
            }
        } else if (mcVer(1_009) && d == EntityDamageEvent.DamageCause.DRAGON_BREATH) {
            deathmsg = this.formatV(
                    e,
                    death_tag = "mob.EnderDragonBreath",
                    this.getMessage("death-messages.mob.EnderDragonBreath"),
                    "");
        } else if (d == EntityDamageEvent.DamageCause.MELTING) {
            deathmsg = this.formatV(
                    e, death_tag = "natural.Melting", this.getMessage("death-messages.natural.Melting"), "");
        } else if (d == EntityDamageEvent.DamageCause.CUSTOM) {
            deathmsg =
                    this.formatV(e, death_tag = "natural.Custom", this.getMessage("death-messages.natural.Custom"), "");
        } else if (d == EntityDamageEvent.DamageCause.SUICIDE) {
            deathmsg = this.formatV(
                    e, death_tag = "natural.Suicide", this.getMessage("death-messages.natural.Suicide"), "");
        } else if (mcVer(1_013) && d == EntityDamageEvent.DamageCause.DRYOUT) {
            deathmsg =
                    this.formatV(e, death_tag = "natural.DryOut", this.getMessage("death-messages.natural.DryOut"), "");
            ...
            if (damager != null && isAnvil((FallingBlock) damager)) {
                ...
                if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                    ...
                    TextComponent result =
                            handleCustomMessages(e, p, damager, isPvP, "AnvilKill", " natural.AnvilKill", true);
                    if (result != null) {
                        deathmsg = result;
                        ...
                        deathmsg = this.formatV(
                                ...
                                damager,
                                death_tag = "natural.AnvilKill",
                                this.getMessage("death-messages.natural.AnvilKill"),
                                this.getKillerName(damager, p),
                                this.getKillerName2(damager, p),
                                "");
                    }
                    ...
                    deathmsg = this.formatV(
                            e, death_tag = "natural.Anvil", this.getMessage("death-messages.natural.Anvil"), "");
                }
            } else {
                if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                    if (damager instanceof Player) {
                    ...
                    TextComponent result = handleCustomMessages(
                            e, p, damager, isPvP, "FallingBlockKill", " natural.FallingBlockKill", true);
                    if (result != null) {
                        deathmsg = result;
                        ...
                        deathmsg = this.formatV(
                                ...
                                damager,
                                death_tag = "natural.FallingBlockKill",
                                this.getMessage("death-messages.natural.FallingBlockKill"),
                                this.getKillerName(damager, p),
                                this.getKillerName2(damager, p),
                                "");
                    }
                    ...
                    deathmsg = this.formatV(
                            e,
                            death_tag = "natural.FallingBlock",
                            this.getMessage("death-messages.natural.FallingBlock"),
                            "");
                }
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result = handleCustomMessages(
                        e, p, damager, isPvP, contactType + "Kill", " natural." + contactType + "Kill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural." + contactType + "Kill",
                            this.getMessage("death-messages.natural." + contactType + "Kill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e,
                        death_tag = "natural." + contactType,
                        this.getMessage("death-messages.natural." + contactType),
                        "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "FireBlockKill", " natural.FireBlockKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.FireBlockKill",
                            this.getMessage("death-messages.natural.FireBlockKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e, death_tag = "natural.FireBlock", this.getMessage("death-messages.natural.FireBlock"), "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "FireTickKill", " natural.FireTickKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.FireTickKill",
                            this.getMessage("death-messages.natural.FireTickKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e, death_tag = "natural.FireTick", this.getMessage("death-messages.natural.FireTick"), "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "CrammingKill", " natural.CrammingKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.CrammingKill",
                            this.getMessage("death-messages.natural.CrammingKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e, death_tag = "natural.Cramming", this.getMessage("death-messages.natural.Cramming"), "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "DrowningKill", " natural.DrowningKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.DrowningKill",
                            this.getMessage("death-messages.natural.DrowningKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e, death_tag = "natural.Drowning", this.getMessage("death-messages.natural.Drowning"), "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result = handleCustomMessages(
                        e, p, damager, isPvP, "PotionWitherKill", " natural.PotionWitherKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.PotionWitherKill",
                            this.getMessage("death-messages.natural.PotionWitherKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e,
                        death_tag = "natural.PotionWither",
                        this.getMessage("death-messages.natural.PotionWither"),
                        "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result = handleCustomMessages(
                        e, p, damager, isPvP, "PotionPoisonKill", " natural.PotionPoisonKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.PotionPoisonKill",
                            this.getMessage("death-messages.natural.PotionPoisonKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e,
                        death_tag = "natural.PotionPoison",
                        this.getMessage("death-messages.natural.PotionPoison"),
                        "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "LavaKill", " natural.LavaKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.LavaKill",
                            this.getMessage("death-messages.natural.LavaKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg =
                        this.formatV(e, death_tag = "natural.Lava", this.getMessage("death-messages.natural.Lava"), "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "MagmaKill", " natural.MagmaKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.MagmaKill",
                            this.getMessage("death-messages.natural.MagmaKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                deathmsg = this.formatV(
                        e, death_tag = "natural.Magma", this.getMessage("death-messages.natural.Magma"), "");
            }
            ...
            if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                ...
                TextComponent result =
                        handleCustomMessages(e, p, damager, isPvP, "VoidKill", " natural.VoidKill", true);
                if (result != null) {
                    deathmsg = result;
                    ...
                    deathmsg = this.formatV(
                            ...
                            damager,
                            death_tag = "natural.VoidKill",
                            this.getMessage("death-messages.natural.VoidKill"),
                            this.getKillerName(damager, p),
                            this.getKillerName2(damager, p),
                            "");
                }
                ...
                if (p.getFallDistance() > (64 - p.getLocation().getY())) {
                    deathmsg = this.formatV(
                            e, death_tag = "natural.VoidFall", this.getMessage("death-messages.natural.VoidFall"), "");
                } else {
                    deathmsg = this.formatV(
                            e, death_tag = "natural.Void", this.getMessage("death-messages.natural.Void"), "");
                }
            }
            ...
            if (result != null) {
                ...
                deathmsg = this.formatV(
                        ...
                        damager,
                        death_tag = "natural.Thorns",
                        this.getMessage("death-messages.natural.Thorns"),
                        this.getKillerName(damager, p),
                        this.getKillerName2(damager, p),
                        "");
            }
            ...
            if (damager instanceof TNTPrimed) {
                ...
                if (source instanceof LivingEntity && !selfIgnited) {
                    TextComponent result =
                            handleCustomMessages(e, p, source, isPvP, "TNTKill", " natural.TNTKill", false);
                    if (result != null) {
                        deathmsg = result;
                        ...
                        deathmsg = this.formatV(
                                ...
                                source,
                                death_tag = "natural.TNTKill",
                                this.getMessage("death-messages.natural.TNTKill"),
                                this.getKillerName(source),
                                this.getKillerName2(source),
                                "");
                    }
                    ...
                    if (shooter instanceof Player) {
                        ...
                        if (source instanceof LivingEntity && !selfIgnited) {
                            ...
                            deathmsg = this.formatV(
                                    e, death_tag = "natural.TNT", this.getMessage("death-messages.natural.TNT"), "");
                        }
                    }
                } else {
                    deathmsg = this.formatV(
                ...
                if (this.bedTicks.containsKey(uuid) && (now - this.bedTicks.get(uuid)) < 100L) {
                    deathmsg = this.formatV(
                            e, death_tag = "natural.Bed", this.getMessage("death-messages.natural.Bed"), "");
                } else deathmsg = this.formatV(e, death_tag = "unknown", this.getMessage("death-messages.unknown"), "");
            }
            ...
            if (damager instanceof Creeper) {
                ...
                if (result != null) {
                    ...
                    String csuf = "";
                    if (((Creeper) le).isPowered()) csuf = "Charged";

                    if (le.getCustomName() == null || !passable(le.getCustomName())) {
                        deathmsg = this.formatV(
                                e,
                                death_tag = "mob.Creeper" + csuf,
                                this.getMessage("death-messages.mob.Creeper" + csuf),
                                "");
                    } else {
                        deathmsg = this.formatV(
                                e,
                                death_tag = getNamedMobSection() + ".Creeper" + csuf,
                                this.getMessage("death-messages." + getNamedMobSection() + ".Creeper" + csuf),
                                le.getCustomName());
                    }
                }
                ...
                deathmsg = this.formatV(
                        e,
                        death_tag = "natural.EnderCrystal",
                        this.getMessage("death-messages.natural.EnderCrystal"),
                        "");
                ...
                if (result != null) {
                    ...
                    if (le.getCustomName() == null || !passable(le.getCustomName())) {
                        deathmsg = this.formatV(
                                e,
                                death_tag = "mob.WitherSpawnBoom",
                                this.getMessage("death-messages.mob.WitherSpawnBoom"),
                                "");
                    } else {
                        deathmsg = this.formatV(
                                e,
                                death_tag = getNamedMobSection() + ".WitherSpawnBoom",
                                this.getMessage("death-messages." + getNamedMobSection() + ".WitherSpawnBoom"),
                                le.getCustomName());
                    }
                }
                ...
                if (mcVer(1_016)) {
                    ...
                    if (source instanceof LivingEntity) {
                        ...
                        TextComponent result =
                                handleCustomMessages(e, p, le, isPvP, "WitherSkull", " natural.WitherSkull", false);
                        if (result != null) {
                            deathmsg = result;
                            ...
                            deathmsg = this.formatV(
                                    ...
                                    le,
                                    death_tag = " natural.WitherSkull",
                                    this.getMessage("death-messages.natural.WitherSkull"),
                                    this.getKillerName(le),
                                    this.getKillerName2(le),
                                    "");
                        }
                    }
                }
                        ...
                        if (damager instanceof LivingEntity && !selfIgnited) {
                            ...
                            if (is.getType() == Material.CROSSBOW) {
                                if (is.getItemMeta() != null
                                        && !isEmptyDisplayName(is.getItemMeta().getDisplayName())) {
                                    wn = is.getItemMeta().getDisplayName();
                                } else if (wn == null
                                        && this.fc.getBoolean("show-custom-death-msg-on-all-weapons", false)) {
                            ...
                            if (wn == null) wn = "";
                            TextComponent result = handleCustomMessages(
                                    e, p, damager, isPvP, "TNTKill", " natural.CrossbowFirework" + cfc, false);
                            if (result != null) {
                                deathmsg = result;
                                ...
                                deathmsg = this.formatV(
                                        ...
                                        damager,
                                        death_tag = "natural.CrossbowFirework" + cfc,
                                        this.getMessage("death-messages.natural.CrossbowFirework" + cfc),
                                        this.getKillerName(damager),
                                        this.getKillerName2(damager),
                                        wn);
                            }
                        } else {
                            deathmsg = this.formatV(
                                    e,
                                    death_tag = "natural.Firework",
                                    this.getMessage("death-messages.natural.Firework"),
                                    "");
                        }
                    }
                }
                if (!src) {
                    ...
                    if (damager != null && p.hasMetadata("dmp.lastDamage")) {
                        ...
                        TextComponent result = handleCustomMessages(
                                e, p, damager, isPvP, "FireworkKill", " natural.FireworkKill", true);
                        if (result != null) {
                            deathmsg = result;
                            ...
                            deathmsg = this.formatV(
                                    ...
                                    damager,
                                    death_tag = "natural.FireworkKill",
                                    this.getMessage("death-messages.natural.FireworkKill"),
                                    this.getKillerName(damager, p),
                                    this.getKillerName2(damager, p),
                                    "");
                        }
                ...
                deathmsg =
                        this.formatV(e, death_tag = "natural.TNT", this.getMessage("death-messages.natural.TNT"), "");
            } else {
                Entity source = null;
                try {
                    source = (Entity) p.getMetadata("dmp.lastDamageEnt2").get(0).value();
            ...
            String safeAlways = "";
            if (m == Material.LADDER) safeAlways = "Ladder";
            if (m == Material.VINE) safeAlways = "Vine";
            if (mcVer(1_016) && m == Material.TWISTING_VINES) safeAlways = "TwistingVine";
            if (mcVer(1_016) && m == Material.WEEPING_VINES) safeAlways = "WeepingVine";
            if (mcVer(1_016) && isTrapdoor(m)) safeAlways = "Trapdoor";
            if (mcVer(1_016) && m == Material.SCAFFOLDING) safeAlways = "Scaffolding";
            String safeKill = safeAlways;
            if (p.hasMetadata("dmp.lastDamageExpl")) safeKill = "Explosion";
            if (!p.hasMetadata("dmp.lastDamage")
                    || !p.getMetadata("dmp.lastDamage").get(0).asString().equalsIgnoreCase("[mob]ENDER_PEARL"))
                damager = resolveDamager(p, damager);
            if (p.hasMetadata("dmp.lastDamage")) {
                ...
                if (p.getMetadata("dmp.lastDamage").get(0).asString().equalsIgnoreCase("[mob]ENDER_PEARL")) {
                    deathmsg = this.formatV(
                            e,
                            death_tag = "natural.FallShort",
                            this.getMessage("death-messages.natural.FallShort"),
                            "");
                    ...
                    TextComponent result =
                            handleCustomMessages(e, p, damager, isPvP, "FallKill", " natural.FallKill", true);
                    if (result != null) {
                        deathmsg = result;
                        ...
                        if (safeKill.isEmpty()) {
                            ...
                            isPvP = true;
                            pvphm.put(pu, true);
                            if (damager instanceof Player) this.lastkiller = (Player) damager;
                            ItemStack is3;
                            if (damager instanceof LivingEntity
                                    && ((LivingEntity) damager).getEquipment() != null
                                    && getItemInMainHand(((LivingEntity) damager).getEquipment()) != null) {
                                String cn = null;
                                ...
                                if (cn != null) {
                                    ...
                                    deathmsg = this.formatV(
                                            e,
                                            damager,
                                            death_tag = "natural.FallKillWeapon",
                                            this.getMessage("death-messages.natural.FallKillWeapon"),
                                            this.getKillerName(damager, p),
                                            this.getKillerName2(damager, p),
                                            wpn2,
                                            is3);
                        ...
                        if (deathmsg == null)
                            deathmsg = this.formatV(
                                    e,
                                    damager,
                                    death_tag = "natural.Fall" + safeKill + "Kill",
                                    this.getMessage("death-messages.natural.Fall" + safeKill + "Kill"),
                                    this.getKillerName(damager, p),
                                    this.getKillerName2(damager, p),
                                    "");
                    }
                ...
                deathmsg = this.formatV(
                        e,
                        death_tag = "natural.Fall" + safeAlways,
                        this.getMessage("death-messages.natural.Fall" + safeAlways),
                        "");
            } else if (mcVer(1_013) && m == Material.WATER) {
                deathmsg = this.formatV(
                        e, death_tag = "natural.FallWater", this.getMessage("death-messages.natural.FallWater"), "");
                ...
                deathmsg = this.formatV(
                        e, death_tag = "natural.FallFire", this.getMessage("death-messages.natural.FallFire"), "");
            } else if (p.getLocation().getY() > 1
                    && p.getWorld().getBlockAt(p.getLocation().add(0, -1, 0)).getType() == Material.CACTUS) {
                deathmsg = this.formatV(
                        e, death_tag = "natural.FallCacti", this.getMessage("death-messages.natural.FallCacti"), "");
            } else if (mcVer(1_014)
                    && p.getLocation().getY() > 1
                    && (p.getWorld().getBlockAt(p.getLocation().add(0, -1, 0)).getType() == Material.SWEET_BERRY_BUSH
                            || p.getWorld().getBlockAt(p.getLocation()).getType() == Material.SWEET_BERRY_BUSH)) {
                deathmsg = this.formatV(
                        e,
                        death_tag = "natural.FallCacti",
                        this.getMessage("death-messages.natural.FallBerryBush"),
                        "");
            } else if (p.getFallDistance() > 5.0f) {
                deathmsg = this.formatV(
                        e, death_tag = "natural.FallLong", this.getMessage("death-messages.natural.FallLong"), "");
            } else {
                deathmsg = this.formatV(
                        e, death_tag = "natural.FallShort", this.getMessage("death-messages.natural.FallShort"), "");
            }
                ...
                if (sht != null) {
                    if (sht instanceof Witch) {
                        ...
                        deathmsg = this.formatV(
                                e,
                                death_tag = reason,
                                this.getMessage("death-messages." + reason),
                                getLEName(le2.getCustomName()));
                        ...
                        if (result != null) {
                            ...
                            deathmsg = this.formatV(
                                    ...
                                    le3,
                                    death_tag = reason,
                                    this.getMessage("death-messages.pvp.PlayerPotion"),
                                    le3.getName(),
                                    le3.getDisplayName(),
                                    "");
                        }
                        ...
                        deathmsg = this.formatV(
                                e,
                                death_tag = "natural.PotionHarming",
                                this.getMessage("death-messages.natural.PotionHarming"),
                                "");
                    }
                    if (sht instanceof LivingEntity) {
                        ...
                        TextComponent result = handleCustomMessages(
                                ...
                                isPvP,
                                "Potion",
                                (reason == null || reason.isEmpty()) ? "natural.PotionHarming" : reason,
                                true);
                        if (result != null) {
                            deathmsg = result;
                            death_tag = reason;
            ...
            if (p.hasMetadata("dmp.lastDamageEnt2")) {
                ...
                if (damager instanceof Projectile) {
                    ...
                    pjt = (EntityType)
                            p.getMetadata("dmp.lastDamageEntT").get(0).value();
                }
            }
            ...
            if (pjt == null) {
                ...
                if (sht2 instanceof BlockProjectileSource) {
                    ...
                    if (useCustomWeapon) {
                        ...
                        noUseItem = false;
                        // deathmsg = this.formatV(e, ply, death_tag = reason,
                        // this.getMessage("death-messages.pvp.Player" + part + "Custom"), ply.getName(),
                        // ply.getDisplayName(), wpn, getItemInMainHand(ply.getInventory()));
                    ...
                    switch (pjt) {
                        case TRIDENT: {
                            ...
                            String cn2;
                            if (is2 == null
                                    || is2.getItemMeta() == null
                                    || isEmptyDisplayName(is2.getItemMeta().getDisplayName()))
                            ...
                            if (le4.getCustomName() == null || !passable(le4.getCustomName())) {
                                ...
                                deathmsg = this.formatV(
                                        e,
                                        death_tag = reason,
                                        this.getMessage("death-messages.mob.DrownedTrident" + customfooter),
                                        "",
                                        cn2,
                                        is2);
                                break;
                            ...
                            deathmsg = this.formatV(
                                    e,
                                    death_tag = reason,
                                    this.getMessage("death-messages." + getNamedMobSection() + ".DrownedTrident"
                                            + customfooter),
                                    le4.getCustomName(),
                                    cn2,
                                    is2);
                        ...
                        default: {
                            reason = "unknown";
                            deathmsg =
                                    this.formatV(e, death_tag = reason, this.getMessage("death-messages.unknown"), "");
                            break;
                        }
                    }
                    ...
                    switch (pjt) {
                        ...
                        case SPECTRAL_ARROW: {
                            ...
                            ItemStack is2 = getItemInMainHand(le4.getEquipment());
                            String cn2;
                            if (!this.fc.getBoolean("show-custom-death-msg-on-all-weapons", false)
                                    && (is2 == null
                                            || is2.getItemMeta() == null
                                            || isEmptyDisplayName(
                            ...
                            if (le4.getCustomName() == null || !passable(le4.getCustomName())) {
                                ...
                                deathmsg = this.formatV(
                                        e,
                                        death_tag = reason,
                                        this.getMessage("death-messages.mob.SkeletonArrow" + customfooter),
                                        "",
                                        cn2,
                                        is2);
                                break;
                            ...
                            deathmsg = this.formatV(
                                    ...
                                    death_tag = reason,
                                    this.getMessage(
                                            "death-messages." + getNamedMobSection() + ".SkeletonArrow" + customfooter),
                                    le4.getCustomName(),
                                    cn2,
                                    is2);
                            break;
                        ...
                        case SPECTRAL_ARROW: {
                            ...
                            if (le4.getCustomName() == null || !passable(le4.getCustomName())) {
                                ...
                                deathmsg = this.formatV(
                                        e,
                                        death_tag = reason,
                                        this.getMessage("death-messages.mob.PillagerArrow" + customfooter),
                                        "",
                                        cn2,
                                        is2);
                                break;
                            ...
                            deathmsg = this.formatV(
                                    ...
                                    death_tag = reason,
                                    this.getMessage(
                                            "death-messages." + getNamedMobSection() + ".PillagerArrow" + customfooter),
                                    le4.getCustomName(),
                                    cn2,
                                    is2);
                            break;
            ...
            if (deathmsg == null) {
                if (pjt == EntityType.ARROW || pjt == EntityType.SPECTRAL_ARROW) {
                    ...
                    deathmsg = this.formatV(
                            e, death_tag = reason, this.getMessage("death-messages.natural.UnknownArrow"), "");
                } else {
                    reason = "unknown";
            ...
            if (damager instanceof LivingEntity) {
                ...
                || (mcVer(1_009) && d == EntityDamageEvent.DamageCause.ENTITY_SWEEP_ATTACK)) {
            ...
            if (mcVer(1_016) && damager instanceof WitherSkull) {
                ...
                if (ps instanceof Player) {
                    ...
                    reason = "natural.WitherSkull";
                    deathmsg = this.formatV(
                            e, death_tag = reason, this.getMessage("death-messages.natural.WitherSkull"), "");
                ...
                if (ps instanceof Player) {
                    ...
                    if (ps != null) {
                        ...
                        if (le6.getCustomName() != null && passable(le6.getCustomName())) {
                            ...
                            deathmsg = this.formatV(
                                    e,
                                    death_tag = getNamedMobSection() + ".EnderDragonBreath",
                                    this.getMessage("death-messages." + getNamedMobSection() + ".EnderDragonBreath"),
                                    le6.getCustomName());
                        } else
                            deathmsg = this.formatV(
                                    e,
                    ...
                    reason = "natural.PotionHarming";
                    deathmsg = this.formatV(
                            e, death_tag = reason, this.getMessage("death-messages.natural.PotionHarming"), "");
                } else {
                    deathmsg = this.formatV(e, death_tag = "unknown", this.getMessage("death-messages.unknown"), "");
                }
            }
            ...
            if (damager instanceof Bat) {
                ...
                if (fangs) reason = "natural.EvokerFang";
                else reason = getReasonForMob("Evoker", le);
            ...
            } else if (damager instanceof Horse
                    && ((mcVer(1_011) && damager.getType() == EntityType.HORSE) || is_horse)) {
            ...
            if (damager instanceof Player) {
                ...
                deathmsg = this.formatV(
                        ...
                        le,
                        death_tag = reason,
                        this.getMessage("death-messages.pvp.Player" + add),
                        le.getName(),
                        ((Player) le).getDisplayName(),
                        wpn2,
                        is3);
            }
        ...
        try {
            ...
            if (deathmsg.getExtra().size() == 1
                    && deathmsg.getExtra().get(0).getInsertion().length() < 1) {
                return;
            }
        } catch (Exception ex) {
        }
    ...
    private Entity resolveDamager(Player p, Entity damager) {
        ...
        if (nDamager != damager && nDamager != null)
            p.setMetadata(
                    "dmp.lastDamage",
                    new FixedMetadataValue(this.pl, "[mob]" + nDamager.getType().toString()));
        return damager;
    }
    ...
    private boolean isTrapdoor(Material m) {
        return m == Material.OAK_TRAPDOOR
    ...
    private boolean passable(String customName) {
        if (pl.config.getBoolean("heart-compat-mode", false)) {
            char[] hearts =
                    pl.config.getString("heart-characters", "â™¡â™¥â�¤â– ").toCharArray();
            for (char arg0 : hearts) if (customName.indexOf(arg0) >= 0) return false;
        }
        return true;
    }

    // add to message queue
    private void queueBroadcastDeath(
            final TextComponent deathmsg,
            final Player plr,
            final World world,
            final boolean isPvP,
            final String vdeathmsg,
            final int prefixlen,
            final int suffixlen) {
        ...
        if (this.fc.getBoolean("death-message-compat-mode")) {
            while (dmqi.hasNext()) {
                ...
                if (dmc.containsKey(d.v.getUniqueId()))
                    this.broadcastDeath(
                            new TextComponent(TextComponent.fromLegacyText(dmc.get(d.v.getUniqueId()))),
                            d.v,
                            d.w,
                            d.pvp,
                            d.vd,
                            d.prel,
                            d.sufl);
                else this.broadcastDeath(d.d, d.v, d.w, d.pvp, d.vd, d.prel, d.sufl);
                dmqi.remove();
            }
        } else {
            while (dmqi.hasNext()) {
        ...
        if (x.startsWith("[mob]")) {
            ...
            if (!mcVer(1_011)) {
                if (damager.getType() == EntityType.SKELETON) {
                    ...
                    @SuppressWarnings("deprecation")
                    Skeleton.SkeletonType skt_w = Skeleton.SkeletonType.WITHER;
                    if (skt == skt_w) x = "WITHER_SKELETON";
                    else if (mcVer(1_010)) {
                        @SuppressWarnings("deprecation")
                        Skeleton.SkeletonType skt_s = Skeleton.SkeletonType.STRAY;
                        if (skt == skt_s) x = "STRAY";
                    }
                } else if (damager.getType() == EntityType.ZOMBIE) {
                    @SuppressWarnings("deprecation")
                    boolean z_v = ((Zombie) damager).isVillager();
                    if (z_v) x = "ZOMBIE_VILLAGER";
                    else if (isHusk(damager)) x = "HUSK";
                }
            }
        ...
        if (!mcVer(1_011)) {
            if ((damager instanceof Skeleton) && ((Skeleton) damager).getSkeletonType() == Skeleton.SkeletonType.WITHER)
                x = "WITHER_SKELETON";
                    ...
                    && ((Skeleton) damager).getSkeletonType() == Skeleton.SkeletonType.STRAY) x = "STRAY";
            else if (damager.getType() == EntityType.ZOMBIE && ((Zombie) damager).isVillager()) x = "ZOMBIE_VILLAGER";
            else if (damager.getType() == EntityType.ZOMBIE && isHusk(damager)) x = "HUSK";
        }
        x = this.mobNameConfigurate(x);
        try {
            final Object o = this.fc.getString("mob-names." + x);
    ...
    void broadcastDebug(final String deathmsg, final Player victim, final World world) {
        ...
        if (this.fc.getBoolean("world-groups._enabled") && (radius < 0)) {
            for (final String s :
                    this.fc.getConfigurationSection("world-groups").getKeys(false)) {
                List<String> ls =
                        this.fc.getConfigurationSection("world-groups").getStringList(s);
                if (ls.contains(world.getName())) {
                    for (final String t : ls) {
                        ...
                        try {
                            for (final Player p2 :
                                    this.pl.getServer().getWorld(t).getPlayers()) {
    ...
    void broadcastDeath(
        ...
        for (final UUID u : alwaysShow) {
            ...
            if (!(this.pl.showdeath.containsKey(p.getUniqueId())
                    && this.pl.showdeath.get(p.getUniqueId())
                    && !p.equals(victim))) this.sendMessage(p, deathmsg);
        }
        alwaysHide.addAll(alwaysShow);
        if (this.pl.dmpban.contains(victim.getUniqueId()) || this.pl.tempban.containsKey(victim.getUniqueId())) {
            if (!alwaysHide.contains(victim.getUniqueId())) this.sendMessage(victim, deathmsg);
            return;
        }
        if (world != null) {
        ...
        if (!this.fc.getBoolean("console-death-message-even-if-disabled", true)) {
            if (this.fc.getString("console-death-message", "normal").equalsIgnoreCase("normal")) {
                ...
                if (this.fc.getBoolean("console-death-message-strip-prefix", false)) {
                    s = s.substring(
                            this.fc.getString("death-messages.prefix", "").length());
                    s = s.substring(
                            0,
                            s.length()
                                    - this.fc
                                            .getString("death-messages.suffix", "")
                                            .length());
                }
                if (this.fc.getBoolean("console-death-message-strip-colors", false)) {
                    s = ChatColor.stripColor(s);
            ...
            if (!br.isCancelled())
                for (final Player p : this.pl.getServer().getOnlinePlayers()) {
                    if (!(this.pl.showdeath.containsKey(p.getUniqueId())
                                    && this.pl.showdeath.get(p.getUniqueId())
                                    && !p.equals(victim))
                            && (!alwaysHide.contains(p.getUniqueId()))) this.sendMessage(p, deathmsg);
                }
            return;
        }
        if (world == null) {
        ...
        if (isPvP && (pvpradius >= 0 || (radius >= 0 && !pl.pvpradius.containsKey(world.getName())))) {
            ...
            pl.getServer().getPluginManager().callEvent(br);
            for (final Player p : world.getPlayers()) {
                if (br.isCancelled()) break;
                if (!(this.pl.showdeath.containsKey(p.getUniqueId())
                                && this.pl.showdeath.get(p.getUniqueId())
                                && !p.equals(victim))
                        && (!alwaysHide.contains(p.getUniqueId())))
                    if (p.getLocation().distanceSquared(vloc) <= checkradius) this.sendMessage(p, deathmsg);
            }
            ...
            for (final Player p : world.getPlayers()) {
                ...
                if (!(this.pl.showdeath.containsKey(p.getUniqueId())
                                ...
                                && !p.equals(victim))
                        && (!alwaysHide.contains(p.getUniqueId())))
                    if (p.getLocation().distanceSquared(vloc) <= radius) this.sendMessage(p, deathmsg);
            }
        } else {
            DeathMessageBroadcastEvent br = new DeathMessageBroadcastEvent(dmid, deathmsg, victim, world, isPvP);
            pl.getServer().getPluginManager().callEvent(br);
                ...
                if (ls.contains(world.getName())) {
                    for (final String t : ls) {
                        ...
                        try {
                            ...
                            DeathMessageBroadcastEvent br =
                                    new DeathMessageBroadcastEvent(dmid, deathmsg, victim, w, isPvP);
                            pl.getServer().getPluginManager().callEvent(br);
                            if (!br.isCancelled()) {
                                for (final Player p2 : w.getPlayers()) {
                                    ...
                                    if (!(this.pl.showdeath.containsKey(p2.getUniqueId())
                                                    && this.pl.showdeath.get(p2.getUniqueId())
                                                    && !p2.equals(victim))
                                            && (!alwaysHide.contains(p2.getUniqueId()))) this.sendMessage(p2, deathmsg);
                                    ps.add(p2.getName());
                                }
                            }
                        } catch (Exception ex) {
    ...
    public void onPlayerDeath_HIGHEST(final PlayerDeathEvent e) {
        ...
        if (this.fc.getBoolean("death-message-compat-mode", true)) {
            if (e.getDeathMessage() == null) {
                ...
                queue.removeIf(dm ->
                        dm == null || dm.v.getUniqueId().equals(e.getEntity().getUniqueId()));
                return;
            }
            if (dmc.containsKey(e.getEntity().getUniqueId())
                    && dmc.get(e.getEntity().getUniqueId()).equals(e.getDeathMessage())) {
                ...
                if (pl.getConfig().getBoolean("death-message-conflict-broadcast", true)) {
                    ...
                    // remove from DMP queue
                    queue.removeIf(dm -> dm == null
                            || dm.v.getUniqueId().equals(e.getEntity().getUniqueId()));
                }
            }
        }
    }
